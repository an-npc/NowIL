...
def get_teams_data(
    ...
    # Subquery that gets NIL values per player
    player_nils = (
        select
        (
...
def get_team_info(
    ...
    # Repeat subquery to get the latest NIL values per player for possible use cases for team data (there is probably a way better way to do this)
    player_nils = (
        select
        (
            Performance.player_id,
    ...
    # Subquery that gives player count and NIL sum per team
    team_stats = (
        select
        (
            Player.team_id,
